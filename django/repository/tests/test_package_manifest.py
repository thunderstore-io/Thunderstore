...
from repository.models import UploaderIdentity, UploaderIdentityMember, UploaderIdentityMemberRole, PackageVersion
...
@pytest.mark.django_db
def test_manifest_v1_serializer_duplicate_dependency(user, manifest_v1_data, package_version):
    UploaderIdentityMember.objects.create(
        user=user,
        identity=package_version.owner,
        role=UploaderIdentityMemberRole.owner,
    ...
    assert serializer.is_valid() is False
    assert len(serializer.errors["non_field_errors"]) == 1
    assert "Cannot depend on multiple versions of the same package" in str(serializer.errors["non_field_errors"][0])
...
@pytest.mark.parametrize(
    ...
    [
        ["1.0.0", ""],
        ["asdasdasd", "Version numbers must follow the Major.Minor.Patch format (e.g. 1.45.320)"],
        ["", "This field may not be blank."],
        ["1.5.2", ""],
        ["1.6.2+post.dev1", "Version numbers must follow the Major.Minor.Patch format (e.g. 1.45.320)"],
        ["11111.111111.111", ""],
        [None, "This field may not be null."],
        [
            "111111.111111.111",
