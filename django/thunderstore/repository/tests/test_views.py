...
def test_package_list_view(client, community_site, ordering):
    for i in range(4):
        uploader = NamespaceFactory.create(
            name=f"Tester_{i}",
        )
...
def test_package_detail_version_view_cannot_be_viewed_by_user(
    ...
    non_namespace_team = TeamFactory()

    team_member.team = non_namespace_team
    team_member.save()
...
def test_package_detail_version_view_get_object(
    ...
    with pytest.raises(Http404) as excinfo:
        view = PackageVersionDetailView(
            kwargs={"owner": owner, "name": name, "version": version},
...
def test_package_download_view(
    user, client, community_site, manifest_v1_package_bytes, namespace
):
    ...
    version = form.save()
    assert version.package.owner == namespace
