...
SITE_DEFINITIONS: List[Tuple[str, str]] = [
    ("localhost", "Thunderstore"),
    ("auth.localhost", "Thunderstore Auth"),
]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if community is None:
            raise CommandError(
                "Unable to find a community to bind sites to. "
                "Populate data or pass --skip-test-data if you prefer manual setup."
            )
        ...
        Site.objects.filter(domain="*").delete()

        # Bind localhost to the community
        localhost = Site.objects.get(domain="localhost")
