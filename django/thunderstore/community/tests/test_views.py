...
def test_package_detail_view(client, active_package_listing, community_site):
    ...
    response = client.get(active_package_listing.get_absolute_url(), HTTP_HOST=community_site.site.domain)
...
def test_package_dependants_view(client, active_package_listing, community_site):
    response = client.get(active_package_listing.dependants_url, HTTP_HOST=community_site.site.domain)
    assert response.status_code == 200
